/* FILE:    SSD1306_128x32_I2C.h
   DATE:    06/07/23
   VERSION: 1.0
   AUTHOR:  Andrew Davies
   
06/07/23 version 1.0: Original version

This library adds hardware support to the HCDisplay library for SSD1306 based screens.
Current supported boards:


0.9" I2C IIC 128x32 SSD1306 OLED Display (HCMODU0118 & HCMODU0119). Available for purchase 
form our store here:

https://hobbycomponents.com/displays/909-9-128-x-32-serial-i2c-uoled-display-white
https://hobbycomponents.com/displays/910-09-128-x-32-serial-i2c-uoled-display-blue
...
#define SSD1306_128_32_RAM_PAGES  (SSD1306_128_32_GRAM_PAGE_END - SSD1306_128_32_GRAM_PAGE_START + 1)
#define SSD1306_128_32_BYTE_ROWS  (SSD1306_128_32_RES_Y / 8)
...
*/
void Display::DInit(uint8_t add)
{
	_I2C_Add = add;
...
void Display::DUpdateDisplay(void)
{
	...
	_Send_Command(SSD1306_128_32_GRAM_COL_START);
	_Send_Command(SSD1306_128_32_GRAM_COL_END);
	 
	/* set graphics ram start and end pages */ 
	_Send_Command(SETPAGEADDRESS);
	_Send_Command(SSD1306_128_32_GRAM_PAGE_START);
	_Send_Command(SSD1306_128_32_GRAM_PAGE_END);


	for (RowIndex = 0; RowIndex < SSD1306_128_32_RAM_PAGES; RowIndex++)
	{
		...
		do
		{
			...
			for(byte i = 0; i < 31 && ColIndex < SSD1306_128_32_RES_X; i++)
			{
		...
		}while(ColIndex < SSD1306_128_32_RES_X);
	}
}
...
void Display::DClear(void)
{
	...
	for (RowIndex = 0; RowIndex < SSD1306_128_32_BYTE_ROWS; RowIndex++)   
