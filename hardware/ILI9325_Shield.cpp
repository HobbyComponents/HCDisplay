...
Open-Smart 2.4 Inch TFT shield with touch screen (HCARDU0108)
...
void Display::DInit(void)
{
	...
	// Write initial setting tot he screen
	_Init_Screen();
...
void Display::_Init_Screen(void)
{
	...
	LCD_CtrlWrite_ILI9325(0x0003, 0x1028); // set GRAM write direction and BGR=1.
	...
	delay(50); // Delay 50ms
	LCD_CtrlWrite_ILI9325(0x0013, 0x1600); // Set VDV[4:0] for VCOM amplitude
	LCD_CtrlWrite_ILI9325(0x0029, 0x0018); // Set VCM[5:0] for VCOMH
	...
	LCD_CtrlWrite_ILI9325(0x0031, 0x0404);
	LCD_CtrlWrite_ILI9325(0x0032, 0x0304);
	LCD_CtrlWrite_ILI9325(0x0035, 0x0005);
	LCD_CtrlWrite_ILI9325(0x0036, 0x1604);
	LCD_CtrlWrite_ILI9325(0x0037, 0x0304);
	LCD_CtrlWrite_ILI9325(0x0038, 0x0303);
	LCD_CtrlWrite_ILI9325(0x0039, 0x0707);
	LCD_CtrlWrite_ILI9325(0x003C, 0x0500);
	LCD_CtrlWrite_ILI9325(0x003D, 0x000F);
	//------------------ Set GRAM area ---------------//
...
/* Resets the interface by writing 3 sets of 0x00's */
...
void Display::DSleep(boolean mode)
{
	...
	}else
	{
		...
		LCD_CtrlWrite_ILI9325(0x0011, 0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
		delay(50); // Delay 50ms
		LCD_CtrlWrite_ILI9325(0x0012, 0x001A); //Inernal reference voltage =Vci;
		delay(50); // Delay 50ms
...
void Display::DScreen(boolean mode)
{
	...
	if(mode)
		LCD_CtrlWrite_ILI9325(0x0007, 0x0133);	//Screen on
	else
		LCD_CtrlWrite_ILI9325(0x0007, 0x0131);  //Screen off
...
void Display::DFlip(uint8_t mode)
{
	switch(mode)
	{
		case(SCREEN_NORMAL):
			...
			_res_x = RES_X;
			_res_y = RES_Y;
			LCD_CtrlWrite_ILI9325(0x0003, 0x1028); 
			_CS_DIS;
			break;
		...
		case(SCREEN_R90):
			...
			_res_x = RES_Y;
			_res_y = RES_X;
			LCD_CtrlWrite_ILI9325(0x0003, 0x1000);
			_CS_DIS;
			break;
		...
		case(SCREEN_R180):
			...
			_res_x = RES_X;
			_res_y = RES_Y;
			LCD_CtrlWrite_ILI9325(0x0003, 0x1018);
			_CS_DIS;
			break;
		...
		case(SCREEN_R270):
			...
			_res_x = RES_Y;
			_res_y = RES_X;
			LCD_CtrlWrite_ILI9325(0x0003, 0x1030);
			_CS_DIS;
			break;
...
/*void Display::DBurstStart(uint16_t SizeX, uint16_t SizeY)
{
	_CS_EN;
	_SetWriteArea(_X1Pos, _Y1Pos, _X1Pos + SizeX, _Y1Pos + SizeY);
	_WriteCommand(RAMWR); //Write to RAM
}

void Display::DBurstPlotRGB(uint8_t r, uint8_t g, uint8_t b)
{
	
	_WriteData(((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3)); 
}

void Display::DBurstEnd(void)
{
	_CS_DIS;
}*/

/*void Display::DPlotRGBBuffer(BufferRGB *Buffer, uint16_t SizeX, uint16_t SizeY)
{
	uint8_t r;
	uint8_t g;
	uint8_t b;
	...
	for(int16_t y = _Y1Pos; y < _Y1Pos + SizeY; y++)
		for(int16_t x = _X1Pos; x < _X1Pos + SizeX; x++)
		{
			r = Buffer[x + (SizeY * y)].r;
			g = Buffer[x + (SizeY * y)].g;
			b = Buffer[x + (SizeY * y)].b;
			_WriteCommand(RAMWR); //Write to RAM
			_WriteData(((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3)); 
		Serial.println(r);
		}
...
void Display::DReadPixelRGB(uint16_t x, uint16_t y, uint8_t *Data)
{
	...
	//Select pixel
	LCD_CtrlWrite_ILI9325(GRAM_HA, x); //GRAM X axis address  
	LCD_CtrlWrite_ILI9325(GRAM_VA, y); //GRAM Y axis address
...
void Display::_SetWriteArea(uint16_t X1, uint16_t Y1, uint16_t X2, uint16_t Y2)
{
	...
	LCD_CtrlWrite_ILI9325(HSA, X1); 	//X axis start address 
	LCD_CtrlWrite_ILI9325(HEA, X2); 	//X axis end address  
	LCD_CtrlWrite_ILI9325(VSA, Y1); 	//Y axis start address  
	LCD_CtrlWrite_ILI9325(VEA, Y2); 	//Y axis end address    
	LCD_CtrlWrite_ILI9325(GRAM_HA, x);	//GRAM X axis address  
	LCD_CtrlWrite_ILI9325(GRAM_VA, y);	//GRAM Y axis address	
}
...
void Display::_WriteCommand(uint16_t command)
{
	_RS_COMMAND;
	
	_WRITE_DATA(command >> 8);
	_WRITE_DATA(command);
}
...
void Display::_WriteData(uint16_t data)
{
	_RS_DATA;
...
boolean Touch::TPressed(void)
{
	...
	if(analogRead(TOUCH_Y_ANALOGUE) < (max(_Cal_X_Min, _Cal_X_Max) / 2))
		state = false;
