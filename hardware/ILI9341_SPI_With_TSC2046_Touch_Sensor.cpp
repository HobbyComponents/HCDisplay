...
void Touch::TInit(uint8_t DIN, uint8_t DOUT, uint8_t CLK, uint8_t CS, uint16_t Cal_X_Min, uint16_t Cal_X_Max, uint16_t Cal_Y_Min, uint16_t Cal_Y_Max, uint8_t Mapping)
{
	...
	_DIN =  portOutputRegister(digitalPinToPort(DIN));
	_DOUT =  portInputRegister(digitalPinToPort(DOUT));
	_CLK =  portOutputRegister(digitalPinToPort(CLK));
	_CS =  portOutputRegister(digitalPinToPort(CS));
	
	_DINM =  digitalPinToBitMask(DIN);
	_DOUTM =  digitalPinToBitMask(DOUT);
	_CLKM =  digitalPinToBitMask(CLK);
	_CSM =  digitalPinToBitMask(CS);
...
uint16_t Touch::TReadRaw(boolean Axis)
{
	...
	if(Axis == TOUCH_AXIS_Y)
		_OutputCommand(0b10011000);
	else 
		_OutputCommand(0b11011000);
