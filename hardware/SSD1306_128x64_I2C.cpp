/* FILE:    SSD1306_128X64_I2C.cpp
   DATE:    22/06/22
   VERSION: 0.1
   AUTHOR:  Andrew Davies
   
22/06/22 version 0.1: Original version

This library adds hardware support to the HCDisplay library for SSD1306 based screens.
Current supported boards:


0.96 I2C IIC OLED SSD1306 - White (SKU: HCMODU0164). Available for purchase form our store here:

https://hobbycomponents.com/displays/1160-096-i2c-iic-oled-module
...
#define SSD1306_RAM_PAGES  (SSD1306_GRAM_PAGE_END - SSD1306_GRAM_PAGE_START + 1)
...
void Display::DUpdateDisplay(void)
{
	...
	_Send_Command(SSD1306_GRAM_COL_START);
	_Send_Command(SSD1306_GRAM_COL_END);
	 
	/* set graphics ram start and end pages */ 
	_Send_Command(SETPAGEADDRESS);
	_Send_Command(SSD1306_GRAM_PAGE_START);
	_Send_Command(SSD1306_GRAM_PAGE_END);


	for (RowIndex = 0; RowIndex < SSD1306_RAM_PAGES; RowIndex++)
	{
		{	
			...
			do
			{
				...
				for(byte i = 0; i < 31 && ColIndex < SSD1306_RES_X; i++)
				{
					Wire.write(DisplayBuffer[ColIndex][RowIndex]); 
					ColIndex++;
				}
				Wire.endTransmission();
			}while(ColIndex < SSD1306_RES_X);
		}
	}
}
