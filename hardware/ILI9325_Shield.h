...
#ifndef ILI9325_SHIELD_h
#define ILI9325_SHIELD_h
#include "Arduino.h"
...
/* ILI9325 command registers used in this library */
#define GRAM_HA	0x20
#define GRAM_VA 0x21
#define RAMWR	0x22
#define HSA		0x50
...
#define _READ_DATA(d) {_RD_LOW; \
						delayMicroseconds(2); \
						d = (PIND & DATA2_MASK) | (PINB & DATA1_MASK); \
						_RD_HIGH;}
...
class Display
{
  public:
  Display(void);
  void DInit(void /*uint8_t DC, uint8_t CS, uint8_t RST*/);
  ...
  void DBurstStart(uint16_t SizeX, uint16_t SizeY);
  void DBurstPlotRGB(uint8_t r, uint8_t g, uint8_t b);
  void DBurstEnd(void);
  ...
  void LCD_CtrlWrite_ILI9325(uint16_t command, uint16_t data);
